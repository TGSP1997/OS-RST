...
butter   = Filter(Filter_Enum.BUTTERWORTH, [1,freq])
...
freq_min_white = minimize(filter_cost,freq,args=(t, y_white, x, butter ,cost))
x_hat_min_white = butter.filter_fun(t,y_white,para = [freq_min_white.x,butter.parameters[1]])
cost_white = cost.cost(x_hat_min_white,x)
standard_cost_white = cost.cost(y_white,x)

freq_min_brown = minimize(filter_cost,freq,args=(t, y_brown, x, butter ,cost))
x_hat_min_brown = butter.filter_fun(t,y_brown,para = [freq_min_brown.x,butter.parameters[1]])
cost_brown = cost.cost(x_hat_min_brown,x)
standard_cost_brown = cost.cost(y_brown,x)

freq_min_quant = minimize(filter_cost,freq,args=(t, y_quant, x, butter ,cost))
x_hat_min_quant = butter.filter_fun(t,y_quant,para = [freq_min_quant.x,butter.parameters[1]])
...
'Butterworth Filter (White Noise)',
'Butterworth Filter (Brown Noise)',
'Butterworth Filter (Quantisation)',
