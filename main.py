...
# noise generation
noise_std_dev = 0.1
noises  = [Noise(Noise_Enum.WHITE, noise_std_dev, seed=i) for i in range(10)]                                    # oder PINK, BROWN, QUANT
noise   = noises[0]
...
cost    = Cost(Cost_Enum.AVG_LOSS)                                                                                   # oder AVG_LOSS, PHASE_SHIFT
...
time, true_input, true_input_dot = sine.get_fun()                                                               # oder polynom oder exp
norm_freq = time[:round(len(time)/2)] / (time[-1] - time[0])
noisy_input = noise.apply_noise(true_input)

# unfiltered differentiation of noisy signal
diff_finite = fwd_diff(time, noisy_input)

# PT1 smoothing
# optimize pt1 parameter
param_pt1 = minimize(filter_cost,0.1,args=(time, noisy_input, true_input, pt1,cost))
param_pt1 = param_pt1.x
# apply PT1
y_hat_pt1 = pt1.filter_fun(time, noisy_input, para = param_pt1)
...
y_hat_wiener = wiener.filter_fun(time, noisy_input, noise_std_dev)
...
kalman_process_noise = minimize(kalman_filter_cost, kalman_process_noise, args=(time, noisy_input, [kalman_filter_order, x_start_guess, noise_std_dev], true_input, kalman, cost), method='Nelder-Mead')
kalman_process_noise = kalman_process_noise.x
# apply Kalman filter
y_hat_kalman = kalman.filter_fun(time, noisy_input, para=[kalman_filter_order, x_start_guess, noise_std_dev, kalman_process_noise])
...
plot_sub.plot_sig(time, [true_input, noisy_input, y_hat_wiener], ["true sine", "noisy sine", "Wiener smoothed"])
plot_sub.plot_sig(time, [noisy_input, y_hat_kalman, y_hat_dot_kalman], ["y", "Kalman", "dot Kalman"])
plt.show()